...
const Navbar: FC = () => {
        ...
        <div className="relative lg:hidden flex space-x-4 items-center">
          ...
          {isOpen && (
            <div className=" text-xl gap-10 fixed top-16  w-full -left-4 h-screen bg-white text-black z-20 flex flex-col justify-center items-center">
              <div>{!session && (<Link href="/login"><div className=" px-32 py-2 cursor-pointer hover:bg-gray-200 rounded w-full text-center" onClick={() => setIsOpen(false)}>Sign in</div></Link>)}</div>
              <div>{!session && (<Link href="/register"><div className="py-2 px-32 cursor-pointer hover:bg-gray-200 rounded w-full text-center" onClick={() => setIsOpen(false)}>Sign up</div></Link>)}</div>
              <Link href="/topgames"><div className="py-2 cursor-pointer hover:bg-gray-200 rounded w-full px-32 text-center" onClick={() => setIsOpen(false)}>Top Games</div></Link>
              <div>{session && (<Link href={`/profile/${session.user.name}`}><div className="py-2 px-32 cursor-pointer hover:bg-gray-200 rounded w-full text-center" onClick={() => setIsOpen(false)}>{session.user.name}</div></Link>)}</div>
  ...
  >
    {session.user.name}
          ...
          <div className='relative'>{isUserMenuOpen && (
            <div ref={userMenuRef} className="absolute top-10 right-6 mt-2 w-48 bg-gray-900 text-white rounded-lg shadow-xl">
              <div onClick={()=>{setIsUserMenuOpen(false)}} className="py-2 text-center cursor-pointer hover:bg-gray-200 hover:text-black">
                {session && (
              ...
              <div onClick={()=>{setIsUserMenuOpen(false)}} className="py-2 cursor-pointer hover:bg-gray-200 hover:text-black">
                {session && <SignOut/>}
              </div>
            </div>
          )}</div>
